...
def preprocessing(directory, window_size=100, target_std=10):
    ...
    for k in range(len(wav_list)):
        ...
        # fragment wav file according to window_size
        wav_fragment_length = int(wav.size/window_size)
        wav = wav[:wav_fragment_length * window_size]

        for i in range(wav_fragment_length):
            torch.save(torch.from_numpy(wav[i * window_size:i * window_size + window_size]), "./TEST_PT/" + name_list[k] + "_" + str(i + 1) +  ".pt")
